...
import DeleteRoleButton from "@/components/dashboard/DeleteRoleButton";
      ...
      <Container>
        ...
        <div className="mt-10 overflow-x-auto rounded-xl border border-cream-dark bg-white/60 shadow-sm">
          <table className="w-full min-w-[640px] text-left">
            ...
            <tbody>
              {roles.map((role) => (
                <tr key={role.id} className="border-b border-cream-dark/60">
                  <td className="p-3 text-sm text-charcoal-soft">{role.order}</td>
                  <td className="p-3 text-charcoal">{role.title}</td>
                  <td className="p-3 text-sm text-charcoal-soft">
                    {role.reportsTo}
                  </td>
                  <td className="p-3 text-right text-sm">
                    <Link
                      href={`/dashboard/committee/${role.id}`}
